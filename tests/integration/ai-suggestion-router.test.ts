...
describe('aiSuggestion router', () => {
  ...
  it('returns validated suggestions built from the available menu', async () => {
    const table = await db.insertInto('Table').values({ id: createId(), label: 'T1', qrToken: 'tok-1' }).returningAll().executeTakeFirstOrThrow();
    ...
    const item = await db.insertInto('MenuItem').values({ id: createId(), name: 'Latte', price: 28000, categoryId: category.id, available: true }).returningAll().executeTakeFirstOrThrow();
    await db.insertInto('MenuItem').values({ id: createId(), name: 'Hidden', price: 10000, categoryId: category.id, available: false }).execute();
  ...
  it('sends every request in the batch together and preserves result order', async () => {
    const table = await db.insertInto('Table').values({ id: createId(), label: 'T-bulk', qrToken: 'tok-bulk' }).returningAll().executeTakeFirstOrThrow();
    const category = await db.insertInto('Category').values({ id: createId(), name: 'Coffee', sortOrder: 1 }).returningAll().executeTakeFirstOrThrow();
    const latte = await db.insertInto('MenuItem').values({ id: createId(), name: 'Latte', price: 28000, categoryId: category.id, available: true }).returningAll().executeTakeFirstOrThrow();
    const tea = await db.insertInto('MenuItem').values({ id: createId(), name: 'Chamomile', price: 17000, categoryId: category.id, available: true }).returningAll().executeTakeFirstOrThrow();
    mockedFetch
  ...
  it('rejects an empty requests array', async () => {
    const table = await db.insertInto('Table').values({ id: createId(), label: 'T-empty', qrToken: 'tok-empty' }).returningAll().executeTakeFirstOrThrow();
    const anon = appRouter.createCaller({ db, user: null, cooldownStore: new RedisCooldownStore() });
    await expect(
  ...
  it('enforces the per-table cooldown on a second immediate request, even across a whole batch', async () => {
    const table = await db.insertInto('Table').values({ id: createId(), label: 'T2', qrToken: 'tok-2' }).returningAll().executeTakeFirstOrThrow();
    const category = await db.insertInto('Category').values({ id: createId(), name: 'Tea', sortOrder: 1 }).returningAll().executeTakeFirstOrThrow();
    await db.insertInto('MenuItem').values({ id: createId(), name: 'Chamomile', price: 17000, categoryId: category.id, available: true }).execute();
  ...
  it('returns no suggestions without calling OpenAI when the menu is empty', async () => {
    const table = await db.insertInto('Table').values({ id: createId(), label: 'T3', qrToken: 'tok-3' }).returningAll().executeTakeFirstOrThrow();
    const anon = appRouter.createCaller({ db, user: null, cooldownStore: new RedisCooldownStore() });
    const result = await anon.aiSuggestion.getSuggestion({
      tableToken: table.qrToken,
      requests: [{ type: 'Coffee', taste: [], aroma: [], texture: [] }],
  ...
  it('surfaces a clear error when any request in the batch fails', async () => {
    const table = await db.insertInto('Table').values({ id: createId(), label: 'T4', qrToken: 'tok-4' }).returningAll().executeTakeFirstOrThrow();
    const category = await db.insertInto('Category').values({ id: createId(), name: 'Coffee', sortOrder: 1 }).returningAll().executeTakeFirstOrThrow();
    await db.insertInto('MenuItem').values({ id: createId(), name: 'Latte', price: 28000, categoryId: category.id, available: true }).execute();
