...
describe('kitchen router', () => {
  ...
  it('moves order to READY once all items are READY', async () => {
    ...
    const items = await db.insertInto('OrderItem')
      .values([
    ...
    let refreshed = await db.selectFrom('Order').selectAll().where('id', '=', order.id).executeTakeFirstOrThrow();
    expect(refreshed.status).toBe('SENT_TO_KITCHEN');

    await kitchen.kitchen.updateItemStatus({ orderItemId: items[1].id, status: 'READY' });
    refreshed = await db.selectFrom('Order').selectAll().where('id', '=', order.id).executeTakeFirstOrThrow();
