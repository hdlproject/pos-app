...
describe('order router', () => {
  ...
  async function seedMenu() {
    ...
    return db.insertInto('MenuItem').values({ id: createId(), name: 'Latte', price: 4.5, categoryId: category.id }).returningAll().executeTakeFirstOrThrow();
  }
  ...
  it('rejects order creation for an item that is auto-out-of-stock even though the manual flag is still available', async () => {
    ...
    const refreshed = await db.selectFrom('MenuItem').selectAll().where('id', '=', item.id).executeTakeFirstOrThrow();
  ...
  it('rejects an append with the wrong table token', async () => {
    const item = await seedMenu();
    await db.insertInto('Table').values({ id: createId(), label: 'T1', qrToken: 'tok-1' }).execute();
    const anon = appRouter.createCaller({ db, user: null });
